...
func (a *sqlAnalyzer) AnalyzeLineage(req *analyzer.AnalyzeReq) ([]*analyzer.LineageResult, error) {
	...
	for _, stmt := range statements {
		...
		if lineageResult != nil {
			// 检查是否是CREATE TEMPORARY TABLE或CREATE TEMPORARY VIEW语句
			if (lineageResult.StmtType == analyzer.StmtTypeCreateTemporaryTable || lineageResult.StmtType == analyzer.StmtTypeCreateTemporaryView) && len(lineageResult.Writes) > 0 {
				// 记录临时表或视图
...
// 参考：https://spark.apache.org/docs/latest/sql-ref-syntax-ddl-alter-table.html#examples
func (a *sqlAnalyzer) MakeCommentModification(req *analyzer.MakeCommentModificationReq) (string, error) {
	...
	if databaseName != "" {
		commentStmt = fmt.Sprintf("ALTER TABLE `%s`.`%s` ALTER COLUMN `%s` COMMENT '%s';", databaseName, tableName, req.ColumnName, req.Comment)
	} else {
		commentStmt = fmt.Sprintf("ALTER TABLE `%s` ALTER COLUMN `%s` COMMENT '%s';", tableName, req.ColumnName, req.Comment)
	}
