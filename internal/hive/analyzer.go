...
func (a *sqlAnalyzer) MakeCommentModification(req *analyzer.MakeCommentModificationReq) (string, error) {
	...
	if databaseName != "" {
		commentStmt = fmt.Sprintf("ALTER TABLE %s.%s CHANGE COLUMN %s %s %s", databaseName, tableName, req.ColumnName, req.ColumnName, targetColumn.Type)
	} else {
		commentStmt = fmt.Sprintf("ALTER TABLE %s CHANGE COLUMN %s %s %s", tableName, req.ColumnName, req.ColumnName, targetColumn.Type)
	}
